...
/**
 * Sends Email Verification using Resend API (Primary/Serverless) or Nodemailer (SMTP)
 */
export async function sendVerificationEmail(
  ...
  try {
    ...
    // 1. If RESEND_API_KEY configured, use Resend REST API
    if (resendApiKey) {
      const fromEmail = process.env.RESEND_FROM_EMAIL || 'KiraPuasaKu <onboarding@resend.dev>';
    ...
    if (!transporter) {
      return {
        success: false,
        error: 'RESEND_API_KEY atau SMTP_USER/SMTP_PASS belum dikonfigurasi dalam persekitaran pelayan (.env).',
      };
    }
    ...
    return { success: false, error: err.message || 'Gagal menghantar emel pengesahan.' };
  }
}

/**
 * Test Connection (Resend or SMTP)
 */
export async function testSMTPConnection(options?: SMTPOptions): Promise<{ success: boolean; message: string }> {
  try {
    ...
    if (resendApiKey) {
      ...
      if (res.ok) {
        return { success: true, message: 'Sambungan Resend API berjaya disahkan!' };
      }
      return { success: false, message: 'Kunci RESEND_API_KEY tidak sah atau telah tamat tempoh.' };
    }
    ...
    if (!transporter) {
      return {
        success: false,
        message: 'Maklumat RESEND_API_KEY atau SMTP_USER dan SMTP_PASS tidak lengkap.',
      };
    }
    ...
    return {
      success: false,
      message: err.message || 'Gagal menyambung ke pelayan emel. Sila semak tetapan.',
    };
  }
}

...
export async function onRequestPost(context: { request: Request; env: Env }) {
  try {
    ...
    const { username, email, name, avatar } = body as any;

    if (!username || !email) {
      return new Response(
        JSON.stringify({ error: 'Sila masukkan nama pengguna dan emel.' }),
    ...
    const displayName = (name && String(name).trim()) || cleanUser;

    const verificationToken = 'ver_' + Date.now() + '_' + Math.random().toString(36).substring(2, 10);
    ...
    const hasEmailConfig = Boolean(env.RESEND_API_KEY);

    return new Response(
      JSON.stringify({
        success: true,
        message: hasEmailConfig && emailSent
          ? `Pendaftaran berjaya! Emel pengesahan telah dihantar ke ${cleanEmail}. Sila semak inbox atau spam emel anda.`
          : 'Pendaftaran akaun berjaya direkodkan.',
        email: cleanEmail,
        username: cleanUser,
        requiresEmailVerification: hasEmailConfig,
        verificationToken,
        emailSent,
        emailError,
        smtpConfigured: hasEmailConfig,
        user: {
          id: 'usr_' + Date.now(),
          username: cleanUser,
          name: displayName,
          email: cleanEmail,
          email_verified: !hasEmailConfig,
          role: 'user',
          status: hasEmailConfig ? 'pending' : 'approved',
    ...
    return new Response(
      JSON.stringify({ error: err.message || 'Ralat semasa memproses pendaftaran.' }),
